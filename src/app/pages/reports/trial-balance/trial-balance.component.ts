...
export class TrialBalanceComponent {
  ...
  getReports() {
    ...
    if(this.myGroup.value.endDate != '')
        ...
        this.companyService.fetchReport('trialbalance-report',params,values).subscribe(res => {
            if (res.code == "1") {
				...
				var res = alasql('SELECT site,opening,debit,credit,closing FROM ? order by item LIMIT '+this.limit+' OFFSET 0',[this.alldata]);
                this.pdata = res;				
            }
        })
    }
	...
	myFuncPrevious(){
		if(this.pageno > 1)
		{
			...
			var res = alasql('SELECT site,opening,debit,credit,closing FROM ? order by item LIMIT '+this.limit+' OFFSET '+((this.pageno - 1) * this.limit)+'',[this.alldata]);
            this.pdata = res;
		}
	}
	...
	excelDownload(){
		var mystyle = {
			...
			columns: [
			  {columnid:'site',title:'Description'},
			  {columnid:'opening'},
			  {columnid:'debit'},			  
			  {columnid:'credit'},			  
			  {columnid:'closing'},
		...
		alasql('SELECT * INTO XLS("Trial_Balance_Reports.xls",?) FROM ?',[mystyle,this.alldata]);
	}
	...
	myFuncSort(colname:string){
		...
		this.pageno = 1;
		var res = alasql('SELECT site,opening,debit,credit,closing FROM ? order by '+colname+' '+this.orderby+' LIMIT '+this.limit+' OFFSET '+((this.pageno - 1) * this.limit)+'',[this.alldata]);
        this.pdata = res;
	}
}
