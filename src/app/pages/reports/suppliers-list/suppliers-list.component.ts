...
export class SuppliersListComponent {
  ...
  getReports() {
        ...
        this.companyService.fetchReport('suppliers-report',params,values).subscribe(res => {
            if (res.code == "1") {
				...
				var res = alasql('SELECT name,address,emailId,phoneNumber,paymentTerms,currency,paymentMode,invoiceNumber,invoiceDate,invoiceAmount,totalAmount,dueDate,amountPaid,dueAmount FROM ? order by vendorName LIMIT '+this.limit+' OFFSET 0',[this.alldata]);
                this.pdata = res;				
            }
        })
    }
	...
	myFuncPrevious(){
		if(this.pageno > 1)
		{
			...
			var res = alasql('SELECT name,department,address,emailId,phoneNumber,fax,paymentTerms,currency,paymentMode,invoiceNumber,invoiceDate,invoiceAmount,totalAmount,dueDate,amountPaid,dueAmount FROM ? order by vendorName LIMIT '+this.limit+' OFFSET '+((this.pageno - 1) * this.limit)+'',[this.alldata]);
            this.pdata = res;
		}
	}
	...
	excelDownload(){
		var mystyle = {
			sheetid: 'Suppliers List Report',
			headers: true,
			caption: {
			  title:'Suppliers List Report',
			},
			column: {
			  style:'font-size:15px'
			},
			columns: [
				{columnid:'name',title:'Vendor Name'},
				{columnid:'address'},
				{columnid:'emailId'},			  
		...
		alasql('SELECT * INTO XLS("Suppliers_List_Report.xls",?) FROM ?',[mystyle,this.alldata]);
	}
	...
	myFuncSort(colname:string){
		...
		this.pageno = 1;
		var res = alasql('SELECT name,address,emailId,phoneNumber,paymentTerms,currency,paymentMode FROM ? order by '+colname+' '+this.orderby+' LIMIT '+this.limit+' OFFSET '+((this.pageno - 1) * this.limit)+'',[this.alldata]);
        this.pdata = res;
	}
